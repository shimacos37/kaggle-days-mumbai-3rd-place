...
class config:
    ...
    bucket_path = "mumbai/sakami/bert_v11/"  # CHECK HERE!!!
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[BatchEncoding, float]:
        data = (
            ...
            self.category[self.indices[idx]],
            self.numerical[self.indices[idx]],
        )
...
class PaddingCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        category = torch.LongTensor(batches[2]).to(self.device)
        numerical = torch.FloatTensor(batches[3]).to(self.device)

        if len(batches) == 5:
            y_batch = torch.LongTensor(batches[4]).to(self.device)
            return category, numerical, x_batch, y_batch

        indices = list(batches[0])
        return indices, category, numerical, x_batch
...
class BertClassifier(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.cat_embedding = nn.Embedding(5, 16)
        self.feature_linear = nn.Linear(16 + 3, 32)
        self.classifier = nn.Linear(config.hidden_size * 4 + 32, 3)
    ...
    def forward(
        ...
        cat_embedding = self.cat_embedding(category)
        feature_embedding = torch.cat([cat_embedding, numerical], dim=-1)
        feature_embedding = self.feature_linear(feature_embedding)
    ...
    def predict(self, data_loader: DataLoader) -> np.ndarray:
        ...
        with torch.no_grad():
            for idx, category, numerical, batch in tqdm(data_loader, desc="predict", leave=False):
                with autocast(enabled=config.fp16):
                    preds[idx] = self(category, numerical, **batch).detach()
...
def main(debug: bool = False):
    ...
    train_df[["sim1", "sim2", "sim3"]] = train_df[["sim1", "sim2", "sim3"]].astype(np.float32)
    test_df[["sim1", "sim2", "sim3"]] = test_df[["sim1", "sim2", "sim3"]].astype(np.float32)
    ...
    df["lang_code"] = df["lang_code"].astype("category").cat.codes
    train_df = df.iloc[: len(train_df)].reset_index(drop=True)
    ...
    with timer("prepare data"):
        ...
        test_dataset = TextDataset(
            test_encodings, test_df["lang_code"].values, test_df[["sim1", "sim2", "sim3"]].values
        )
    ...
    with timer("train"):
        ...
        for fold, (train_idx, valid_idx) in enumerate(generate_split(train_encodings)):
            ...
            train_dataset = TextDataset(
                train_encodings,
                train_df["lang_code"].values,
                train_df[["sim1", "sim2", "sim3"]].values,
            ...
            for epoch in range(config.n_epochs):
                ...
                for category, numerical, x_batch, y_batch in progress:
                    with autocast(enabled=config.fp16):
                        y_preds = model(category, numerical, **x_batch)
